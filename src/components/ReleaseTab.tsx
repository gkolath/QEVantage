      ...
      <section className="glass-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px', marginBottom: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            <a
              href="https://github.com/gkolath/QEVantage/actions"
              target="_blank"
        ...
        <div className="terminal-container" style={{
          ...
          {releaseLogs.length === 0 ? (
            ...
            releaseLogs.map((log, idx) => {
              ...
              const color = log.includes('CRITICAL') || log.includes('[CRITICAL]')
                ? 'var(--status-danger)'
                : isGithub ? 'var(--accent-blue)'
              ...
              return (
                <div key={idx} style={{ marginBottom: '6px', color }}>
                  ...
                  {urlMatch ? (
                    <>
                      {log.substring(0, log.indexOf(urlMatch[1]))}
                      <a
                        href={urlMatch[1]}
                        target="_blank"
                        rel="noopener noreferrer"
                      ...
                      {log.substring(log.indexOf(urlMatch[1]) + urlMatch[1].length)}
                    </>
                  ) : log}
                </div>
              );
